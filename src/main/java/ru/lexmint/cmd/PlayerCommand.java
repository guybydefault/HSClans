...
public class PlayerCommand extends BaseCommand {
    ...
    public void perform(CommandSender sender, String[] subargs) {
        ...
        if (cpLayer != null) {
            ...
            HSClans.instance.getMessenger().message("commands.player.time-played", sender, String.valueOf(cpLayer.getHoursPlayedRounded()));
            HSClans.instance.getMessenger().message("commands.player.league", sender, cpLayer.getClanLeague().getName());
            if (!cpLayer.hasClan()) {
            ...
            HSClans.instance.getMessenger().message("commands.player.not-found", sender, subargs[1]);
        }
    }
}
