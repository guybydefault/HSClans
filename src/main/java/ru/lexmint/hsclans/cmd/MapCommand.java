 ...
 */
public class MapCommand extends AbstractClanPlayerCommand {
    /**
     * Characters which are used as symbols of the map's legend.
     */
    private final char[] legendCharArray = HSClans.instance.getMessenger().getMessage("commands.map.legend-chars").toCharArray();
     ...
     */
    public MapCommand(String[] aliases, ClanRole requiredClanRole, String permission, int arguments, String usage) {
    ...
    public void perform(Player sender, String[] subargs) {
        for (String row : getMap(HSClans.instance.getClanManager().getPlayer(sender.getName(), true), sender.getLocation())) {
            sender.sendMessage(row);
        }
    }

    private List<String> getMap(CPLayer cpLayer, Location location) {
